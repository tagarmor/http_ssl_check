...
# Candidate ports tried in order — common high ports unlikely to be blocked
CANDIDATE_PORTS = [5151, 5152, 5153, 8181, 8282, 8383, 9191, 9292]

def find_free_port(candidates: List[int]) -> Optional[int]:
    """Return the first port in *candidates* that is free on 127.0.0.1."""
...
if __name__ == "__main__":
    ...
    if PORT is None:
        ...
        print("  Tried: " + ", ".join(str(p) for p in CANDIDATE_PORTS))
        print()
        print("  Possible causes on corporate Windows:")
        print("  - Windows Firewall / corporate policy blocking ports")
        print("  - Antivirus blocking localhost connections")
        print()
        print("  Suggested fixes:")
        print("  1. Run this script as Administrator")
        print("  2. Temporarily disable the firewall for 127.0.0.1")
        print("  3. Ask IT to allow localhost ports 5151-5153 or 8181-9292")
        print("=" * 60 + "\n")
        sys.exit(1)

    # Write the chosen port to a file so the dashboard can read it
